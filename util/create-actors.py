...
def get_color_index(line: str) -> str:
    (color_indices, color_names, color_index) = [
...
with open(
    "LEGO1\\lego\\legoomni\\src\\common\\legoactors.cpp", mode="r", encoding="ascii"
) as f:
    ...
    while offset < len(lines) - 16:
        ...
        (
            ...
            chest_names,
            chest_index,
        ) = [part.strip() for part in lines[offset + 6].strip("{},").split(",")]
        (
            hat_part_indices,
            hat_parts,
            hat_part_index,
            ...
            hat_color_names,
            hat_color_index,
        ) = [part.strip() for part in lines[offset + 7].strip("{},").split(",")]
        ...
        match hat_part_indices:
            case "g_pepperHatPartIndices":
                hat_parts_name = "PEPPER_HAT_PARTS"
            case "g_infomanHatPartIndices":
                hat_parts_name = "INFOMAN_HAT_PARTS"
            case "g_ghostHatPartIndices":
                hat_parts_name = "GHOST_HAT_PARTS"
            case "g_hatPartIndices":
                hat_parts_name = "HAT_PARTS"
            case _:
                raise Exception()
        ...
        (face_texture_indices, face_texture_names, face_texture_index) = [
            part.strip() for part in lines[offset + 9].strip("{},").split(",")
        ][3:]
