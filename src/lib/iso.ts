...
export class IsoReader {
	...
	constructor(buffer: ArrayBuffer, variant: IsoVariant = IsoVariant.Joliet) {
		...
		if (variant === IsoVariant.ISO9660) {
			...
			if (this.getVolumeDescriptorType(17) !== 2) {
				throw new Error(
					"Volume descriptor at sector 17 must be type 2 for Joliet",
				);
			}
		...
		const rootDirRecordBuffer = rootDirRecordBytes.buffer.slice(
			rootDirRecordBytes.byteOffset,
			rootDirRecordBytes.byteOffset + rootDirRecordBytes.byteLength,
		);
	...
	private readDir(start: number, totalLen: number, currentPath: string): void {
		...
		const decoder = new TextDecoder(
			this.variant === IsoVariant.Joliet ? "utf-16be" : "ascii",
		);
		...
		while (n < totalLen) {
			...
			if (
				this.mainReader.position + nameLen > this.mainReader.length ||
				this.mainReader.position + nameLen > recordStartReaderOffset + recLen
			) {
				break;
			}
	...
	open(path: string): ArrayBuffer {
		...
		const bufferSlice = this.view.buffer.slice(
			entry.loc * SECTOR_SIZE,
			entry.loc * SECTOR_SIZE + entry.len,
		);
